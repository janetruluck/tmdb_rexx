...
describe MovieRexx::Client do
  ...
  describe ".company", :vcr do
    let(:resource) { movie_rexx.company("company-id") }
  ...
  describe ".company_movies", :vcr do
    let(:resource) { movie_rexx.company_movies("company-id") }
    it { expect(resource).to respond_to(:id) }
    ...
    context ".results" do
      let(:resource) { movie_rexx.company_movies("company-id").results.first }
      it { expect(resource).to respond_to(:id) }
