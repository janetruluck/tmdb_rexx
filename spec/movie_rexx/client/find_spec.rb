...
describe MovieRexx::Client do
  ...
  describe "invalid external source" do
    it { expect{ movie_rexx.find("some-id", "invalid_source") }.to raise_error(MovieRexx::InvalidExternalSourceError) }
  end

  describe ".find", :vcr do
    let(:resource) { movie_rexx.find("some-id", "imdb_id") }
    ...
    context ".movie_results" do
      let(:resource) { movie_rexx.find("some-id", "imdb_id").movie_results.first }
