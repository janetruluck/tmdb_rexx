...
describe MovieRexx::Client do
  ...
  describe ".discover", :vcr do
    context "movie" do
      let(:resource) { movie_rexx.discover("movie", { primary_release_year: 2015 }) }
      ...
      context ".results" do
        let(:resource) { movie_rexx.discover("movie", { primary_release_year: 2015 }).results.first }
    ...
    context "tv" do
      let(:resource) { movie_rexx.discover("tv", { "first_air_date.lte" => "2015-01-01" }) }
      it { expect(resource).to respond_to(:results) }
      it { expect(resource).to respond_to(:page) }
      it { expect(resource).to respond_to(:total_pages) }
      it { expect(resource).to respond_to(:total_results) }
      ...
      context ".results" do
        let(:resource) { movie_rexx.discover("tv", { "first_air_date.lte" => "2015-01-01" }).results.first }
        it { expect(resource).to respond_to(:backdrop_path) }
